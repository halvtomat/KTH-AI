    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.settings.threshold:
            ...
            while not end_episode:
                ...
                Q[s_current][action] = Q[s_current][action] + self.settings.alpha * (R + self.settings.gamma * np.nanmax(Q[s_next]) - Q[s_current][action])
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
            ...
            diff = np.abs(np.nansum(Q_old) - np.nansum(Q))
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
